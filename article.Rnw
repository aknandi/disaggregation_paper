...
In order to know which pixels (i.e. which rows) are contained in each polygon, a matrix is constructed that contains the start and end pixel index for each polygon (\code{getStartendindex} function). Additionally, an INLA mesh is built to use for the spatial field (\code{build_mesh} function). The \code{mesh.args} argument allows the user to supply a list of INLA mesh parameters to control the mesh used for the spatial field. The mesh can take a while to construct, so to prepare the data without buidling the mesh the user can set the \code{makeMesh} flag to FALSE. However, it would then not be possible to fit the disaggregation model without the mesh.
...
%
<<subplotdata, fig.cap='Maps of Madagascar showing the data used in the disaggregation model. These plots are produced when calling the plot function on the disag.data object.', fig.subcap=c('Number of malaria cases in each administrative unit', 'Maps of the four covariates used in the model', 'inla.mesh object that will be used to make the spatial field'), out.width='0.5\\linewidth', echo=FALSE, fig.ncol = 2, dev = 'png', fig.width = 4, fig.height = 5, fig.pos = 'H'>>=
data_plots$polygon
