...
The data, $y_i$, are assumed to be created by the aggregation of the counts over the polygon, i.e., the count data of the polygon is given by the sum of the count data for all the pixels within that polygon. As well as the model covariates, an aggregation raster can be provided transform from the pixel level predictions (rate) to count data, from which it is trivial to calculate the corresponding polygon value by summing up the values for the individual pixels within the polygon. For example, in epidemiology, we may have as our response the number of people that contract a certain disease in a given period of time (case incidence). Our rate would be the incidence rate of cases per population, where the aggregation raster corresponds to population. Predictions of incident rate per population are made at pixel level, the population (aggregation) raster is used to convert these values to number of cases, which can then be summed to get predicted polygon level cases.
...
with two hyperparameters: $\rho = \frac{\sqrt{8\nu}}{\kappa}$, the range of the field (beyond which correlation is $<$ 0.1), and $\sigma$, the marginal standard deviation, relating to the magnitude of the variation in the field. The parameter $\nu$ is the smoothness and is fixed at 2. The Matern field is at least $m$ times mean square differentiable if and only if $\nu > m$~\citep{stein1999interpolation}, therefore $\nu = 2$ is chosen as it is the smallest integer (least smooth) for the field to be differentiable. The Gamma function is given by $\Gamma(\nu) = (\nu - 1)!$ and $K_{\nu}$ is the modified Bessel function of the second kind.
...
The predictions are made at pixel level, however the response data exists as count data at the polygon level. Therefore, to calculate the contribution  to the likelihood from the data we must aggregate the pixel predictions, (via the aggregation raster $a_{ij}$, to get the predicted polygon count.
...
y_i \sim \textrm{Poisson}(\textrm{cases}_i)
\end{equation}
...
\pkg{TMB} can fit both Frequentist and Bayesian models. In this case we have chosen the Bayesian approach as it is most widely used in the spatial modelling community.
...
We then read in the data to use in the \pkg{disaggregation} package. The \code{shapefile} function and \code{raster} function are contained within the \pkg{raster} package, they are functions to read spatial data and raster data respectively. These functions return objects of type SpatialPolygonsDataFrame and RasterLayer respectively. A SpatialPolygonsDataFrame is an \prolang{R} class that holds spatial data in the form of polygons with attributes, in this case the attributes are number of malaria cases within the polygon and polygon ID. A RasterLayer is an \prolang{R} class that holds a single raster. The function \code{getCovariateRasters} is a helper function contained within the \pkg{disaggregation} package. The user provides a directory name and shapefile, it finds all the raster files within that directory and creates a RasterStack with the same extend as the shapefile provided. In this example we are building a RasterStack of the covariate rasters with the same extent as the population raster. A RasterStack is a collection of RasterLayer objects with the same spatial extent and resolution.
...
An optional aggregation raster can be provided. The aggregation raster defines how the pixels within each polygon are aggregated. The disaggregation model performs a weighted sum of the pixel predictions, weighted by the pixel values in the aggregation raster. In this case our pixel predictions are malaria incidence rate, so we use the population raster to aggregate pixel incidence rate by summing the number of cases (rate weighted by population). If no aggregation raster is provided a uniform distribution is assumed, i.e., the pixel predictions are aggregated to polygon level by summing the pixel values unaltered.
...
To fit a model with a Gaussian field we approximate a Gaussian field using Gaussian Random Markov Fields (GRMF), and solve using the stochastic partial differential equation (SPDE) approach~\citep{Lindgren2011}. This reduces the complexity of the problem to a sparse precision matrix, allowing explicit computation of the GMRF representation. This approach requires building a mesh, i.e. splitting the space into finite elements, and calculating field values at the mesh nodes. If a mesh is too fine, the field values take too long to compute, however if the mesh if too coarse, the resulting field is a poor approximation and leads to mesh artifacts in the predictions. The 2D mesh for the spatial field is built using the \code{build_mesh} function, which makes use of the \pkg{INLA} function \code{inla.mesh.2d}. The user can control the parameters of the mesh, including the granularity, using the argument \code{mesh.args} in the \code{build_mesh} function. The parameters \code{max.edge}, \code{cut} and \code{offset} can be set in the \code{mesh.args} list and are defined within the \code{inla.mesh.2d} function. By providing a two values to the \code{max.edge} parameter, the mesh contains an inner region of finer mesh, and an outer coarse region. This approach allows the region of interest to have a fine a mesh as necessary without also requiring time consuming computations in the sea regions. The mesh can take several minutes to construct, so to prepare the data without building the mesh the user can set the \code{makeMesh} flag to FALSE. However, it would then not be possible to fit the disaggregation model without the mesh. 
...
\item \code{priors}: List of priors to use for the model. For any not set, the default priors will be used.
...
The \code{plot} function can be used on the \code{disag_prediction} objects, as shown in Figure \ref{fig:plotprediction}. From Figure~\ref{fig:plotdata}, it can be seen that the polygon response data does not include the islands, whereas the covariate rasters do. As can be seen in Figure~\ref{fig:plotprediction}, out-of-sample malaria predictions of these islands have been made, however these predictions do not contribute to the in-sample performance in Figure~\ref{fig:plotmodel}.
...
\begin{tabular}{l|ll|ll}
 & \multicolumn{2}{|c|}{MCMC (71 hours)} & \multicolumn{2}{|c}{TMB (56 seconds)} \\
Parameter & Mean & SD & Mean & SD \\
\hline
Intercept & -3.22 & 0.33 & -3.20 & 0.27 \\
Slope 1 & -0.47 & 0.20 & -0.47 & 0.20 \\
Slope 2 & 0.41 & 0.22 & 0.39 & 0.22 \\
Slope 3 & 0.21 & 0.29 & 0.21 & 0.29 \\
$log(\tau_u)$ & 0.90 & 0.27 & 0.98 & 0.27 \\
$log(\sigma)$ & 0.062 & 0.009 & -0.002 & 0.166 \\
