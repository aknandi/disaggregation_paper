...
\Abstract{
  Disaggregation modelling, or downscaling, has been used across many fields including epidemiology and ecology. These models use repsonse data agreggated of large heterogenous regions and predict the response at fine-scale over the region by using fine-scale covariates to inform the heterogeneity. This paper presents the \proglang{R} package \emph{disaggregation}, which provides the functionality to streamline the process of running a disaggregation model for fine scale predictions.
}
...
In this section we show how to use the \pkg{disaggregation} package using real malaria data of aggregated malaria case counts across Madagascar. Malaria is a infectious disease caused by parasites of the \emph{Plasmodium} group, transmitted by \emph{Anopheles} mosquitoes. Malaria transmission is therefore closely related to mosquito and parasite development. Environmental factors such as temperature, rainfall and elevation have been shown to have signifcant effect on mosquito survival and development; in general mosquitoes favour warm, humid environments with moderate rainfall. Therefore, such environmental covariates would be useful in a malaria disaggregation model to inform fine-scale distributions. In this model we use the environmental covariates of mean land surface temperature (LST), variation in LST, elevation, and enhanced vegetation index (EVI) to inform spatial hetrogenity in malaria risk. These covaraites are obtained from the Moderate Resolution Imaging Spectroradiometer (MODIS), which provides many measurements over the entire Earth's surface (\url{https://modis.gsfc.nasa.gov/data/}). 
...
The trace of the MCMC parameter values is given in Figures~\ref{fig:mcmclong}. It can be seen that the MCMC algorithm has been run for long enough to get sufficient chain mixing. The \pkg{disaggregation} package produces similar results to the MCMC algorithm, however the model fitting using the \pkg{disaggregation} package only takes 56 seconds. Therefore, it is clear that the \pkg{disaggregation} package provides a quick and simple way to run disaggregation models, that cannot practically be done using MCMC.

%
\begin{table}
\centering
\begin{tabular}{l|ll|ll}
 & \multicolumn{2}{|c|}{MCMC (x hours - TBC)} & \multicolumn{2}{|c}{TMB (56 seconds)} \\
Parameter & Mean & SD & Mean & SD \\
\hline
Intercept & -3.06 & 0.42 & -3.12 & 0.27 \\
Slope 1 & 0.17 & 0.29 & 0.24 & 0.27 \\
Slope 2 & 0.31 & 0.22 & 0.30 & 0.21\\
Slope 3 & -0.34 & 0.22 & -0.37 & 0.20 \\
Slope 4 & -0.24 & 0.26 & -0.24 & 0.21 \\
iid $log(\tau)$ & 1.11 & 0.28 & 1.10 & 0.27 \\
$log(\sigma)$ & -3.35 & 0.60 & -3.55 & 0.59 \\
$log(\rho)$ & 0.74 & 0.34 & -0.56 & 0.30 \\
\end{tabular}
\end{table}
%
...
\begin{figure}[t!]
\centering
\includegraphics[width=\linewidth]{figures/mcmc_long.pdf}
\caption{Trace of the fixed effects parameters for MCMC using NUTS sampling, running the algorithm for x hours.}
\label{fig:mcmclong}
\end{figure}
%
