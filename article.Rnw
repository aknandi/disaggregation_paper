...
This is the approach used by the \code{rasterize} function in the \pkg{raster} package, the \code{fasterize} function in the \pkg{fasterize} package and \code{st_interpolate_aw} from the \pkg{sf} package.
Similarly, for time-series data there are functions such as \code{td} in the \pkg{tempdisagg} package.
...
The popular R package \pkg{INLA} can perform disaggregation regression in the simplest case of ...
