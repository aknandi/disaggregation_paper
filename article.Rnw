...
\Abstract{
  Disaggregation modelling, or downscaling, has been used across many fields including, epidemiology, ecology and something else. These models use repsonse data agreggated of large heterogenous regions and predict the response at fine-scale over the region by using fine-scale covariates to inform the heterogeneity. This paper presents the \proglang{R} package \emph{disaggregation}, which provides the functionality to streamline the process of running a disaggregation model for fine scale predictions.
}
...
Methods for estimating high resolution repsonse information from models of aggregated repsonse data over large spatial regions are 
becoming increasingly sought after in disease risk~\cite{li2012log, diggle2013spatial}, expecially in malaria mapping~\cite{sturrock2014fine, alegana2016advances}, and ecological speices distribution~\cite{keil2013downscaling, barwell2014can}. The aggreagtion of repsonse data over large 
heterongenous regions is problematic for understanding the fine scale distribution. However, by using fine scale information from related covariates we can inform the hetrogeneity of the response variable of interest within the agreggated area.

Disaggregation modelling is unorthodox as the predictions are at a different scale to the response data, i.e. the number of rows in the covariate data is different to that of the repsonse data. Traditional spatial modelling software package, \pkg{INLA}~\cite{rue2009approximate}, has been shown to very useful in a wide variety of circumstances, however it is not flexible enough for the unorthodox nature of the disaggregation problem. Disaggregation models can be implemented in \pkg{TMB} (\cite{tmb2016}) with a lot of flexibility, however the data manipulation required to format the model objects and construct the model definition in C++ is non-trivial. The \pkg{disaggregation} package allows this process to be streamlined, to make it easy for the user to run disaggregation models with some level of flexibility.
...
Let's say we have response data for $N$ polygons, which corresponds to count data for the property of interest within that polygon, $y_j$. By count data here we mean the data at the level that can be aggregated by simple addition, i.e. the count data of the polygon is given by the sum of the count data for all the pixels within that polygon. The rate, $p_i$ is defined such that the number of cases in a pixel can be calculated by multiplying the rate by the aggregation raster. For example, in epidemiology, we may have as our response the number of people with a certain disease (case incidence). Our rate would be the incidence rate as a fraction of the population, where the aggregation raster corresponds to population. 

For the disaggregation model, we first predict the rate, $pred_i$, at pixel level:
%
...
where $X$ are the covariates, $GP$ is the gaussian random field and $u_j$ is the polygon-specific iid random effect. The Gaussian random field has a Matern covariance function, with hyperparameters $\rho = \frac{\sqrt{8\lambda}}{\kappa}$, the range of the field (beyond which correlation is $<$ 0.1), and $\sigma$, the marginal standard deviation, relating the magntiude of the variation of the field. Each of the model parameters and hyperparameters are given priors, which is discussed later. 
...
The different likelihoods correspond to slightly different models ($y_j$ is the repsonse count data):
...
\item \textbf{Binomial} (For a survey in polygon j, $y_j$ is the number positive and $N_j$ is the number tested),
...
Note the normal likelihood is modelled in rate space as it is unreasonable to assume that the dispersion of the likelihood is the same for polygons containing different numbers of cases. Also, the binomial likelihood can be used for survey data, for example when the response data consists of the number of people who tested positive for a disease and the total number of people that were tested.

\subsection{Priors}

For each of the model parameters we specify priors and hyperpriors. The regression parameters and intercept are given Gaussian priors, where the default priors are $\beta_0 \sim N(0, 2)$ and $\beta_i \sim N(0, 0.4)$. 

The field parameters are given penalised complexity priors, $\rho_{min}$ and $\rho_{prob}$, where $P(\rho < \rho_{min}) = \rho_{prob}$, and the variation, $\sigma_{max}$ and $\sigma_{prob}$, where $P(\sigma > \sigma_{max}) = \sigma_{prob}$. The effect of these hyperpriors is to regularise the field to have smaller variations over larger distances. Similarly for the iid effect, the priors are penalised complexity priors on the standard deviation, $sd_{max}$ and $sd_{prob}$, such that $P(sd > sd_{max}) = sd_{prob}$. This has the effect of regularising the iid effect to be small.
...
The \pkg{disaggregation} package is built on Template Model Builder (\pkg{TMB})~\cite{tmb2016}, which is a tool for buidling flexible complex models based on C++. \pkg{TMB} combines the packages CppAD~\cite{bell2012cppad}, for automatic differentiation in C++, Eigen~\cite{guennebaud2010eigen}, a C++ library for linear algebra, and CHOLMOD~\cite{chen2008algorithm}, a package for efficient computation of sparse matrices. The use of these packages allows an efficient implementation of the applied Laplace approximation~\cite{skaug2006automatic} with exact derivatives. \pkg{TMB} calculates first and second order derivatives of the objective function using automatic differentiation~\cite{griewank2008evaluating}.

More about mathematical details of Laplace approximation and automatic differentiation.

The \pkg{disaggregation} package contains a C++ function that defines the model and computes the joint likelihood as a function of the parameters and the random effects, in the format expected by \pkg{TMB}. The \pkg{TMB} package then calculates estimates of both parameters and random effects using the Laplace approximation for the likelihood. Evaluation of the objective function and its derivatives is performed in \proglang{R}.

\section{Package usage} \label{sec:code}

In this section we show how to use the \pkg{disaggregation} package uasing real malaria data of aggregated malaria case counts across Madagascar, and real environmental covariates, such as temperature, rainfall and elevation, to inform spatial hetrogenity in malaria risk. The latest version of \pkg{disaggregation} should always be available from the Comprehensive R Archive Network (CRAN) at \href{http://CRAN.R-project.org/package=disaggregation}{http://CRAN.R-project.org/package=disaggregation}. Run the following commands to install and load the package.
...
We then setup the data to use in the \pkg{disaggregation}.
%
<<setuppaths>>=
...
# Make a RasterStack of covariates to the extent of Madagascar and scale values
covariates <- lapply(covariate_raster_paths, raster)
covariate_stack <- stack(covariates)
covariate_stack <- crop(covariate_stack, extent(c(42, 52, -26, -10))) %>% scale

# Read in the incidence data and only select data from Madagascar 2013
api <- read.csv(API_path)
api <- api %>% filter(iso3 == 'MDG', year == 2013)
...
# Add incidence information to the shapefile (incidence = API * population/1000)
...
In this section we show performance comparisons between modelling using this \pkg{disaggregation} package, based on \pkg{TMB}, and using Markov chain Monte Carlo (MCMC). MCMC algorithms sample the probability space to produce an esimate of the probability distribution. Tese algorithms tend to be slow, as it takes a long time to effectively sample the likelihood space. Using the Laplace approximation, automatic differentiation and efficienct computation of sparse matrices within \pkg{TMB}, the time taken to optimise the model is be considerably shorter. Here we compare the time and performance of the two techniques.

The model described above for malaria in Madagascar is fitted by running MCMC using the \pkg{tmbstan} package, with the NUTS algorithm. The model is fitted three different ways: firstly, the MCMC algorithm is run for 1000 iteration, which takes 8 hours; secondly algorithm is then run for 26 iterations, taking 3 minutes; and thirdly the model is optimised using \pkg{TMB} within the \pkg{disaggregation} package, which takes 56 seconds. Fitted parameter values for all three methods are given in the table below. 
...
# Running the MCMC algorithm for 3 minutes
start <- Sys.time()
mcmc_out <- tmbstan(m$obj, chains = 4, iter = 30, 
                    cores = getOption('mc.cores', 4))
end <- Sys.time()
print(end - start)

# Running the MCMC algorithm for 8 hours
start <- Sys.time()
mcmc_out2 <- tmbstan(m$obj, chains = 4, iter = 1000, 
                     cores = getOption('mc.cores', 4))
end <- Sys.time()
print(end - start)
...
stan_trace(mcmc_out2, pars = c('intercept', 
                               'slope[1]', 'slope[2]', 'slope[3]', 'slope[4]', 
                               'iideffect_log_tau', 'log_sigma', 'log_rho'))
@
%

The trace of the MCMC parameter values for the long and the short experiments is given in Figures~\ref{fig:mcmcshort} and \ref{fig:mcmclong} respectively. It can be seen that running the MCMC algorithm for 3 minutes does not show mixing between the different chain; the algorithm much be run for much longer, for exmaple 8 hours, to get sufficient chain mixing. The \pkg{disaggregation} package produces similar results to the MCMC algorithm that runs for 8 hours, however the model fitting using the \pkg{disaggregation} package only takes 56 seconds. Therefore, it is clear that the \pkg{disaggregation} package provides a quick and simple way to run disaggregation models, that cannot practically be done using MCMC.

%
\begin{table}
\begin{tabular}{l|ll|ll|ll}
 & \multicolumn{2}{|c|}{MCMC (8 hours)} & \multicolumn{2}{|c|}{MCMC (24 seconds)} & \multicolumn{2}{|c}{TMB (56 seconds)} \\
Parameter & Mean & sd & Mean & sd & Mean & sd \\
Intercept & -3.16 & 0.19 & -0.20 & 1.33 & -3.12 & 0.27 \\
Slope 1 & 0.32 & 0.23 & -0.27 & 0.80 & 0.24 & 0.27 \\
Slope 2 & 0.11 & 0.22 & -0.65 & 0.86 & 0.30 & 0.21\\
Slope 3 & -0.55 & 0.18 & -0.35 & 0.95 & -0.37 & 0.20 \\
Slope 4 & -0.23 & 0.16 & 0.12 & 0.35 & -0.24 & 0.21 \\
iid $log(\tau)$ & 1.06 & 0.26 & 0.04 & 1.16 & 1.10 & 0.27 \\
$log(\sigma)$ & -3.29 & 0.61 & -0.87 & 0.94 & -3.55 & 0.59 \\
$log(\rho)$ & 0.75 & 0.34 & 0.65 & 0.93 & -0.56 & 0.30 \\
\end{tabular}
\end{table}
%
...
\begin{figure}[t!]
\centering
\includegraphics[width=\linewidth]{figures/mcmc_short_trace.pdf}
\caption{Trace of the fixed effects parameters for MCMC using NUTS sampling, running the algorithm for 3 minutes.}
\label{fig:mcmcshort}
\end{figure}
%
...
\caption{Trace of the fixed effects parameters for MCMC using NUTS sampling, running the algorithm for 8 hours.}
\label{fig:mcmclong}
\end{figure}
%
...
Disaggregation modelling, which involves predicting at fine-scale resolution using areal data over heterogenous regions, has become widely used in fields such as epidemiology, ecology and something. The \pkg{disaggregation} package implements Bayesian spatial disaggregation modelling with a simple, easy to use \proglang{R} interface. The package includes simple data preparation, fitting and prediction functions that allow some user defined model flexibility. In this paper we have presented an application of the package, predicting malaria incidence rate across Madagascar from aggregated count data and environmental covariates. 
