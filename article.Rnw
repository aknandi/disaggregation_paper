...
Disaggregation regression, was first applied to ecological species distribution modelling~\cite{keil2013downscaling, barwell2014can} but has now become an important method in disease risk mapping \cite{falciparum2019, vivax2019}. 
The aggregation of response data over large heterogenous regions is problematic for understanding the fine-scale distribution. However, by using fine-scale information from related covariates we can inform the hetrogeneity of the response variable of interest within the agreggated area.
