...
%
\begin{itemize}
\item {\textbf{Gaussian} ($\sigma_i$ is the dispersion of the polygon data)
...
\item \textbf{Binomial} (For a survey in polygon $i$, $y_i$ is the number positive cases and $M_i$ is the number tested),
...
covariate_stack <- getCovariateRasters('data/covariates', shape = population_raster)
@
%
